    ...
    <div className="bg-[#FAFBFC] dark:bg-gray-950 min-h-screen">
      ...
      <div className="bg-gradient-to-br from-[#1B1F3B] to-[#2D3366] dark:from-gray-900 dark:to-gray-800 text-white py-16 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="relative max-w-md mx-auto">
            ...
            <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher un article..." className="pl-10 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 h-11 border-gray-200 dark:border-gray-700" />
          </div>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
        ...
        {featured && selectedCat === 'Tous' && !search && (
          <div className="bg-white dark:bg-gray-800/50 rounded-3xl overflow-hidden border border-gray-100 dark:border-gray-700 shadow-sm dark:shadow-gray-900/50 mb-12 flex flex-col lg:flex-row">
            ...
            <div className="p-8 flex flex-col justify-center">
              ...
              <h2 className="text-2xl font-extrabold text-[#1B1F3B] dark:text-gray-100 mb-3">{featured.title}</h2>
              <p className="text-gray-500 dark:text-gray-400 mb-6">{featured.excerpt}</p>
        ...
        <div className="flex gap-2 flex-wrap mb-8">
          {categories.map(cat => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${selectedCat === cat ? 'bg-[#FF6B00] text-white' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-700 hover:border-[#FF6B00] dark:hover:border-orange-500 hover:text-[#FF6B00] dark:hover:text-orange-400'}`}
            >
              {cat}
            </button>
          ))}
        ...
        {others.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {others.map(article => (
              <div key={article.id} className="bg-white dark:bg-gray-800/50 rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-700 hover:shadow-md dark:hover:shadow-gray-900/50 transition-shadow cursor-pointer group">
                ...
                <div className="p-5">
                  ...
                  <h3 className="font-bold text-[#1B1F3B] dark:text-gray-100 mb-2 line-clamp-2">{article.title}</h3>
                  <p className="text-gray-500 dark:text-gray-400 text-sm line-clamp-2 mb-4">{article.excerpt}</p>
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-400 dark:text-gray-500">Aucun article trouvé</p>
          </div>
        )}
      </div>
    </div>
